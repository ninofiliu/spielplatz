...
(async () => {
  ...
  for (let i = 0; i < 10_000; i++) {
    ...
    spiral.move();
    if (spiral.x > canvas.width * (0.5 + 0.5 * spiralsSize))
      spiral.x = ~~(spiral.x - canvas.width * spiralsSize * spiralLoop);
    if (spiral.x < canvas.width * (0.5 - 0.5 * spiralsSize))
      spiral.x = ~~(spiral.x + canvas.width * spiralsSize * spiralLoop);
    if (spiral.y > canvas.height * (0.5 + 0.5 * spiralsSize))
      spiral.y = ~~(spiral.y - canvas.height * spiralsSize * spiralLoop);
    if (spiral.y < canvas.height * (0.5 - 0.5 * spiralsSize))
      spiral.y = ~~(spiral.y + canvas.height * spiralsSize * spiralLoop);
